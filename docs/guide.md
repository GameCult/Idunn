...
From the Idunn repository root:

```powershell
cargo build --release
cargo run --release --bin idunn -- --daemon demo --rudp-health-bind <idunn-rudp-health-bind>
```
...
```powershell
cargo run --release --bin idunn -- --daemon demo --restart-command "echo restart demo"
```
...
```powershell
cargo run --release --bin idunn -- --daemon demo --deploy-command "echo deploy demo" --restart-command "echo restart demo" --execute
```

To keep watching on a resident interval:

```powershell
cargo run --release --bin idunn -- --daemon demo --interval-seconds 30
```

To run the built-in Starfire-local swarm profile directly:

```powershell
cargo run --release --bin idunn -- --swarm-profile starfire-local --repo-root <repo-root> --execute
```
...
```powershell
cargo run --release --bin idunn -- --daemon demo --store E:\path\to\idunn.keepalive.cc
```
